...
chosenGNSS = 1; % enter GPS to output speed and loc of midship
                    % 1: Fore, 2: Aft, 3:3rd
useheading = 1; %  enter the combination of GPS or gyro for compute heading of ship
                         % 0:Gyro, 1:Fore to aft, 2: Fore to 3rd, 3:aft to 3rd (not reccomended because of ship ref. length)
useinitheading =1; %enter the combination of GPS for compute initial heading of ship
...
delta_history = [0.0];
n_history = [0.0];
true_windv_history = [0.0];
true_windd_history = [0.0];
aparent_windv_history = [0.0];
aparent_windd_history=[0.0];
...
%     %initialize values
    ...
    r_GNSS= zeros(height(data),no_GPS);
    ...
    r_kf_dag = zeros(height(data),no_GPS);
    r_kf = zeros(height(data),no_GPS);
    ...
    %%%%
    U_gps_raw = zeros(height(data), no_GPS);
    u_gps_raw_midship = zeros(height(data), no_GPS);
    vm_gps_raw_midship = zeros(height(data), no_GPS);
    beta_from_gps_doppler = zeros(height(data), no_GPS);
    U_gps_raw_midship = zeros(height(data), no_GPS);
%%%%%%%%
% ONLY 14th Sep 2020 data, GPS 2nd and 3rd were swaped because of miss
% coneection.
%%%%%%%%
   ...
   %%
    ...
    for k =1:no_GPS           
       for j =1:height(data)
            ...
            % compute angvelo by GNSS
            if j == 1
            r_GNSS(j,k) = (psi_hat_GNSS(j+1,k) - psi_hat_GNSS(j,k))/dt;
            else
            r_GNSS(j,k) = (psi_hat_GNSS(j,k) - psi_hat_GNSS(j-1,k))/dt;
            end
...
%     end
    ...
    for k = 1: no_GPS
        for j=1:height(data)
            ...
            u_shipfix_lp(j,k) = u_hat_kf(j,k) * cos(psi_hat(j)) + vm_hat_kf(j,k) * sin(psi_hat(j));
            vm_shipfix_lp(j,k) = vm_hat_kf(j,k) * cos(psi_hat(j)) - u_hat_kf(j,k) * sin(psi_hat(j));
    ...
    % convert GPS velocity to midship (written by fueki)
    for k = 1: no_GPS
        for j = 1:height(data)
            if k==1
                U_gps_raw(j,k) = data.GPSSpeedKM_Fore(j)/3.6;
                beta_from_gps_doppler(j,k) = deg2rad(data.GPSTrueNorth_Fore(j)-theta_berth) - psi_hat(j);
                u_gps_raw_midship(j,k) = U_gps_raw(j,k) * cos(beta_from_gps_doppler(j,k));
                vm_gps_raw_midship(j,k) = U_gps_raw(j,k) * sin(beta_from_gps_doppler(j,k)) - L_GPS(1) * deg2rad(data.YawAngVel(j));
                U_gps_raw_midship(j,k) = sqrt(u_gps_raw_midship(j,k)^2 + vm_gps_raw_midship(j,k)^2);
            elseif k==2
                U_gps_raw(j,k) = data.GPSSpeedKM_Aft(j)/3.6;
                beta_from_gps_doppler(j,k) = deg2rad(data.GPSTrueNorth_Aft(j)-theta_berth) - psi_hat(j);
                u_gps_raw_midship(j,k) = U_gps_raw(j,k) * cos(beta_from_gps_doppler(j,k));
                vm_gps_raw_midship(j,k) = U_gps_raw(j,k) * sin(beta_from_gps_doppler(j,k)) - L_GPS(2) * deg2rad(data.YawAngVel(j));
                U_gps_raw_midship(j,k) = sqrt(u_gps_raw_midship(j,k)^2 + vm_gps_raw_midship(j,k)^2);
            elseif k==3
                U_gps_raw(j,k) = data.GPSSpeedKM_3(j)/3.6;
                beta_from_gps_doppler(j,k) = deg2rad(data.GPSTrueNorth_3(j)-theta_berth) - psi_hat(j);
                u_gps_raw_midship(j,k) = U_gps_raw(j,k) * cos(beta_from_gps_doppler(j,k));
                vm_gps_raw_midship(j,k) = U_gps_raw(j,k) * sin(beta_from_gps_doppler(j,k)) - L_GPS(3) * deg2rad(data.YawAngVel(j));
                U_gps_raw_midship(j,k) = sqrt(u_gps_raw_midship(j,k)^2 + vm_gps_raw_midship(j,k)^2);
            end
       end
    end
    ...
    for n = 1:no_anemo
        ...
        v_shipfix_anemo(j,n) = vm_shipfix_kf(j,chosenGNSS) + r_kf(j) * L_anemo(n);
    ...
    legend('Fore', '3rd', 'Aft','location','southeast')
    xlabel('time (s)')
...
%     saveas(gca,figurename,filetype) 
 %% torajectory
   ...
   %%
    ...
    hold on
    plot(data.time,U_GPS_kf(:,1),'r.')
    plot(data.time,data.GPSSpeedKM_Aft/3.6,'b')
    plot(data.time,U_GPS_kf(:,2),'b.')
    plot(data.time,data.GPSSpeedKM_3/3.6,'k')
    plot(data.time,U_GPS_kf(:,3),'k.')
    xlabel("time (s)")
    ylabel("velocity (m/s)")
%     xlim([2000 2100])
    ylim([-1 1])
...
%     plot(data.time, u_shipfix_lp(:,3),'k--')
     plot(data.time, u_gps_raw_midship(:,1),'r.')
     plot(data.time, u_gps_raw_midship(:,2),'b^')
     plot(data.time, u_gps_raw_midship(:,3),'k^')
    xlabel('time (s)')
    ylabel('filetered velocity of midship(m/s)')
    htit=title('ship fix velocity $u$');
    %hleg=legend('kf, Fore','kf, aft',...
        %'kf, 3rd','raw Fore','raw Aft','raw 3rd','location','best','fontsize',14);
    %set(hleg, 'Interpreter','latex')
    set(htit, 'Interpreter','latex') 
    ylim([0 0.2])
    hold off
...
%    plot(data.time, vm_shipfix_lp(:,3),'k--')
    ...
    hold on
    plot(data.time, rad2deg(r_kf),'k--')
%     plot(data.time, rad2deg(r_GNSS(:,1)),'r.')
    xlabel('time (s)')
    ylabel('Ang velocity (deg/s)')
    legend('Gyro output','kalman filtered')
    hold off
...
%     % name variables
    ...
    varNames ={'t','x_position_mid','u_velo', 'y_position_mid', 'vm_velo', 'psi_hat',...
     'r_angvelo', 'n_prop', 'delta_rudder','wind_velo_relative_mid',...
     'wind_dir_relative_mid','wind_velo_true','wind_dir_true',...
     'u_velo_raw','vm_velo_raw','psi','psi_hat_GNSS_f2a','psi_hat_GNSS_f23','psi_hat_GNSS_a23','r_angvelo_raw','chosen_GPS','chosen_headind_method','u_gps_raw_midship', 'vm_gps_raw_midship', 'U_gps_raw_midship'};
    savetable = table(data.time, x_hat(:,chosenGNSS), u_shipfix_kf(:,chosenGNSS), y_hat(:,chosenGNSS), vm_shipfix_kf(:,chosenGNSS), psi_hat,...
    r_kf, n_prop, delta_rudder, UA_midship, gamma_a_midship, UT_ave,zeta_wind,...
    u_shipfix_raw(:,chosenGNSS), vm_shipfix_raw(:,chosenGNSS), psi, psi_hat_GNSS(:,1),psi_hat_GNSS(:,2),psi_hat_GNSS(:,3),...
    deg2rad(data.YawAngVel),typeGPS, typeheading,u_gps_raw_midship(:,chosenGNSS),vm_gps_raw_midship(:,chosenGNSS),U_gps_raw_midship(:,chosenGNSS),'variablenames',varNames); 
    
    savename = ['./processed_csv/berthing_processed_',files(i).name, '.csv'];
    writetable(savetable,savename)
    
    %% make PDF data of control and state variable
%     if (i==3)
%         figure(25)
%         histogram( u_shipfix_kf(:,chosenGNSS),21,'normalization','pdf','facecolor','#000080')
%         hold on
%         
%      figure(26)
%     histogram(vm_shipfix_kf(:,chosenGNSS),21,'normalization','pdf','facecolor','#000080')
%     hold on
%     
%      figure(27)
%     histogram(r_lowpass,21,'normalization','pdf','facecolor','#000080')
%     hold on
%     
%     figure(28)
%     histogram(n_prop,21,'normalization','pdf','facecolor','#000080')
%     hold on
%     
%     figure(29)
%     histogram(delta_rudder,21,'normalization','pdf','facecolor','#000080')
%     hold on
%     
%     figure(30)
%     histogram(UT_ave,21,'normalization','pdf','facecolor','#000080')
%     hold on
%     
%     figure(31)
%     histogram(rad2deg(zeta_wind),21,'normalization','pdf','facecolor','#000080')
%     hold on
%     
%     figure(32)
%     histogram(UA_midship,21,'normalization','pdf','facecolor','#000080')
%     hold on
%     
%     figure(33)
%     histogram(rad2deg(gamma_a_midship),21,'normalization','pdf','facecolor','#000080')
%     hold on
%     
%     else
%         u_history =vertcat(u_history,  u_shipfix_kf(:,chosenGNSS));
%         vm_history = vertcat(vm_history, vm_shipfix_kf(:,chosenGNSS));
%         r_history = vertcat(r_history,  r_lowpass);
%         delta_history = vertcat(delta_history, delta_rudder);
%         n_history = vertcat(n_history, n_prop);
%         true_windv_history = vertcat(true_windv_history,  UT_ave);
%         true_windd_history = vertcat(true_windd_history,  zeta_wind);
%         aparent_windv_history = vertcat(aparent_windv_history, UA_midship);
%         aparent_windd_history = vertcat(aparent_windd_history, gamma_a_midship);
%     end
 end
 %%
%     figure(25)
%     histogram(u_history(2:end),21,'normalization','pdf','facecolor','#4DBEEE')
%     xlabel('\itu \rm(\itm/s\rm)','fontsize',20)
%     ylabel('\itPDF','fontsize',20)
%     xlim([-0.32 0.32])
%     ylim([0 8])
%     legend('test','train','location','best','fontsize',20)
%     hold off
%     %
%      figure(26)
%     histogram(vm_history(2:end),21,'normalization','pdf','facecolor','#4DBEEE')
%     xlabel('\itv_{m} \rm(\itm/s\rm)','fontsize',20)
%     xlim([-0.12 0.12])
%     ylim([0 22])
%     ylabel('\itPDF','fontsize',20)
...
%     %
%      figure(27)
%     histogram(r_history(2:end),21,'normalization','pdf','facecolor','#4DBEEE')
%     set(hleg, 'Interpreter','latex')
%     xlabel('\itr \rm(\itrad/s\rm)','fontsize',20)
%     ylabel('\itPDF','fontsize',20)
%     xlim([-0.06 0.06])
%     ylim([0 35])
%     legend('test','train','location','best')
...
%     %
%     figure(28)
%     histogram(n_history(2:end),21,'normalization','pdf','facecolor','#4DBEEE')
%     set(hleg, 'Interpreter','latex')
%     xlabel('\itn_p \rm(\itrps\rm)','fontsize',20)
%     ylabel('\itPDF','fontsize',20)
%     legend('test','train')
%     xlim([-12 12])
%     ylim([0 0.325])
...
%     %
%     figure(29)
%     histogram(delta_history(2:end),21,'normalization','pdf','facecolor','#4DBEEE')
%     xlabel('\delta \rm(\itrad\rm)','fontsize',20)
%     ylabel('\itPDF','fontsize',20)
%     xlim([-0.8 0.8])
%     ylim([0 10])
%     legend('test','train','location','best')
...
%    
%     figure(30)
%     histogram(true_windv_history(2:end),21,'normalization','pdf','facecolor','#4DBEEE')
%     xlabel('\itU_T \rm(m/s\rm)','fontsize',20)
%     ylabel('\itPDF','fontsize',20)
%     xlim([-0.8 0.8])
%     ylim([0 10])
%   legend('test','train','location','best','fontsize',20)
%      figurename_emf=strcat('./figures/true_velo_.emf');
%     print(gcf,figurename_emf,'-dmeta')
%     hold off
%     
%     figure(31)
%     histogram(rad2deg(true_windd_history(2:end)),21,'normalization','pdf','facecolor','#4DBEEE')
%     xlabel('\zeta_w \rm(\itdegree\rm)','fontsize',20)
%     ylabel('\itPDF','fontsize',20)  
%     figurename_emf=strcat('./figures/true_dir_.emf');
%     print(gcf,figurename_emf,'-dmeta')
%     xlim([-0.8 0.8])
%     ylim([0 10])
%     hold off
%         figure(32)
%     histogram(aparent_windv_history(2:end),21,'normalization','pdf','facecolor','#4DBEEE')
%     xlabel('\itU_A \rm(m/s\rm)','fontsize',20)
%     ylabel('\itPDF','fontsize',20)
%     figurename_emf=strcat('./figures/relative_velo_.emf');
%     print(gcf,figurename_emf,'-dmeta')
%         xlim([-0.8 0.8])
%     ylim([0 10
%     hold off
%     figure(33)
%     histogram(rad2deg(aparent_windd_history(2:end)),21,'normalization','pdf','facecolor','#4DBEEE')
%     xlabel('\gamma_a \rm(\itdegree\rm)','fontsize',20)
%     ylabel('\itPDF','fontsize',20)
%    figurename_emf=strcat('./figures/relative_dir_.emf');
%     print(gcf,figurename_emf,'-dmeta')
%     xlim([-0.8 0.8])
%     ylim([0 10])
% hold off
...
% y(k) = C7x(k)+w(k)
% のシス�?�?行�??
% Q,R:雑音v,wの共�?散行�??, v,wは正規白色雑音で
% E[v(k)]= E[w(k)]=0
% E[v{k}'v{k}]=Q,E[w(k)'w(k)]=R
% であることを想�?
% u: 状態更新前時点での制御入�? u(k-1)
% y: 状態更新後時点での制御入�? y(k)
% xhat, P: 更新前�?�状態推定量 xhat(k-1)・誤差共�?散行�?�P(k-1)
% ----  戻り�?�  ----
% xhat_new: 更新後�?�状態推定�?�xhat(k)
% P_new: 更新後�?�誤差共�?散行�?�P(k)
% G: カルマンゲイン G(k)
% 
% 列�?�クトルに変換
xhat=xhat(:); u=u(:);y=y(:);
...
G = Pm * C/(C' * Pm *C +R);
% 事後推定�?�
xhat_new = xhatm + G * (y-C'*xhatm);% 状�?
P_new = (eye(size(A)) - G * C') * Pm; % 誤差共�?散
end
%%
function [ dist, co ] = Cal_dist_co( ALong, ALat )
%****************************************************************************    
%  compute disntance traveled and course of ship by of 2 latitude and longitude of points using 漸長緯度航�?
